...
*	@public
*	@param {bundle.type.Type} type current type in used
*	@param {astq.Node} [node = {}] node to resolve
*	@return {String}
**/
export const resolvePath = (type, ast, node) => {
	let out = '';
	//console.log('GIVEN NODE: ', node);
	//console.log('PATHS: ', type.collectByElement(ast, 'ImportPath', undefined, ['amd']).get(0));
	return out;
};
...
export const add = (modules, node) => {
	//console.log('AMD Specifier', node.value);
